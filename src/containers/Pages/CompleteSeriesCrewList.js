...
const CompleteCrewList = ({
  ...
  const filterCrew = (list) => {
    ...
    const writing = list.filter(
      (crewMember) => crewMember.department === "Writing"
    );
    const sound = list.filter(
      (crewMember) => crewMember.department === "Sound"
    );
    const camera = list.filter(
      (crewMember) => crewMember.department === "Camera"
    );
  ...
  return (
    <div className="crew-details-section-container">
      <div className="cast-section-container">
        <h3>
          Cast:
          <span className="members-count">{castList && castList.length}</span>
        </h3>
      ...
      <div className="crew-section-container">
        ...
        {crewList.production && (
          <>
            {crewList.production.length === 0 ? (
              ""
            ) : (
              <h4>
                Production:
                <span className="members-count">
                  {crewList.production.length}
                </span>
              </h4>
            )}
          </>
        ...
        {crewList.writing && (
          <>
            {crewList.writing.length === 0 ? (
              ""
            ) : (
              <h4>
                Writing:
                <span className="members-count">{crewList.writing.length}</span>
              </h4>
            )}
          </>
        )}
        ...
        {crewList.sound && (
          <>
            {crewList.sound.length === 0 ? (
              ""
            ) : (
              <h4>
                Sound:
                <span className="members-count">{crewList.sound.length}</span>
              </h4>
            )}
          </>
        )}
        ...
        {crewList.camera && (
          <>
            {crewList.camera.length === 0 ? (
              ""
            ) : (
              <h4>
                Sound:
                <span className="members-count">{crewList.camera.length}</span>
              </h4>
            )}
          </>
        )}
